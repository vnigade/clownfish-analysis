...
output_file="${output_prefix}_${sim_method}_window_${window_size}_${window_stride}_models_${local_sample_size}_${remote_sample_size}_rank_${my_rank}.log"

rm ${output_file}
echo "Log file: ${output_file}"
echo "Local scores dump dir: ./data/scores_dump/${local_model}/sample_duration_${window_size}/image_size_${local_sample_size}/window_${window_stride}/val/"
echo "Remote scores dump dir: ./data/scores_dump/${remote_model}/sample_duration_${window_size}/image_size_${remote_sample_size}/window_${window_stride}/val/"

for (( i=${start_filter_interval}; i<${end_filter_interval}; i++ ))
do
python3 main.py --datasets_dir ${datasets_dir} --local_scores_dir ${datasets_dir}/PKUMMD/scores_dump/${local_model}/sample_duration_${window_size}/image_size_${local_sample_size}/window_stride_${window_stride}/val/ --remote_scores_dir ${datasets_dir}/PKUMMD/scores_dump/${remote_model}/sample_duration_${window_size}/image_size_${remote_sample_size}/window_stride_${window_stride}/val/ --filter_interval $i --remote_lag ${remote_lag} --sim_method ${sim_method} --window_stride ${window_stride} --window_size ${window_size} --siminet_path ${siminet_path} >> ${output_file}
done
